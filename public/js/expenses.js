      ...
      try {
        ...
        for (let i = 0; i < response.data.response.length; i++) {
          if (response.data.response[i].description === elementToDelete) {
            ...
            try {
              await axios.delete(
                "http://localhost:3000/expenses/deleteExpense/" +
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function renderDownloads(page) {
    ...
    const data = await fetchPageData(
      `http://localhost:3000/premium/getDownloads?page=${page}&pageSize=${localStorage.getItem(
        "dwnRows"
      )}`,
      page
    );
  ...
  async function fetch() {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/expenses/getExpenses?page=${expCurrentPage}&pageSize=${localStorage.getItem(
          "expRows"
        )}`,
        {
          headers: { "Authorization": token },
